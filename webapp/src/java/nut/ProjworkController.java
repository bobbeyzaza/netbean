...
public class ProjworkController implements Serializable {
    ...
    @EJB
    private nut.ProjworkFacade ejbFacade;
    ...
    public Projwork getSelected() {
        if (current == null) {
            ...
            current.setProjworkPK(new nut.ProjworkPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public Projwork getProjwork(nut.ProjworkPK id) {
    ...
    public static class ProjworkControllerConverter implements Converter {
        ...
        nut.ProjworkPK getKey(String value) {
            nut.ProjworkPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new nut.ProjworkPK();
            key.setProjno(values[0]);
            key.setEmpnum(values[1]);
            return key;
        }

        String getStringKey(nut.ProjworkPK value) {
